<?php

namespace App\Http\Controllers\Api\P2P;

use App\Constants\Status;
use App\Http\Controllers\Controller;
use App\Models\Currency;
use App\Models\P2P\Ad;
use App\Models\P2P\AdPaymentMethod;
use App\Models\P2P\PaymentMethod;
use App\Models\P2P\PaymentWindow;
use App\Models\P2P\UserPaymentMethod;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class AdvertisementController extends Controller
{
    public function index()
    {
        $ads = Ad::select('p2p_ads.*', "wallets.balance")
            ->publishStatus()
            ->latest('p2p_ads.id')
            ->where('p2p_ads.user_id', auth()->id())
            ->with('asset', 'fiat', 'paymentWindow', 'paymentMethods.paymentMethod')
            ->groupBy('p2p_ads.id')
            ->paginate(getPaginate());

        $notify[] = 'Manage Advertisement';

        return responseSuccess('manage_advertisement', $notify, ['ads' => $ads]);
    }

    public function create($id = 0)
    {
        $step  = request()->step ?? 1;
        $steps = [1, 2, 3];

        if ((!in_array($step, $steps)) || (!$id && $step != 1)) {
            $notify[] = 'Step not found';
            return responseError('step_not_found', $notify);
        };

        if ($id) {
            $ad = Ad::with('asset:id,symbol,image', 'fiat:id,symbol,rate,image', 'asset.marketData:currency_id,symbol,price')->where('id', $id)->where('user_id', auth()->id())->first();

            if (!$ad) {
                $notify[] = 'Ad not found';
                return responseError('not_found', $notify);
            }

        } else {
            $ad = null;
        }

        if ($step == 1) {
            $coins      = Currency::with('marketData:currency_id,price')->crypto()->active()->get();
            $currencies = Currency::fiat()->active()->where('rate', '>', 0)->get();
            $data       = [
                'coins'      => $coins,
                'currencies' => $currencies,
            ];
        } else if ($step == 2) {
            $coinPrice         = @$ad->asset->marketData->price;
            $currencyPrice     = @$ad->fiat->rate;
            $originalPrice     = $coinPrice / $currencyPrice;
            $price             = @$ad->price > 0 ? @$ad->price : $originalPrice;
            $priceMargin       = @$ad->price_margin > 0 ? @$ad->price_margin : 100;
            $paymentMethods    = PaymentMethod::whereJsonContains('supported_currency', $ad->fiat->symbol)->active()->orderBy('name')->get();
            $paymentWindows    = PaymentWindow::active()->orderBy('minute')->get();
            $adPaymentMethodId =  @$ad->paymentMethods->pluck('payment_method_id')->toArray();
            $data              = [
                'coinPrice'         => $coinPrice,
                'currencyPrice'     => $currencyPrice,
                'originalPrice'     => $originalPrice,
                'price'             => $price,
                'priceMargin'       => $priceMargin,
                'paymentMethods'    => $paymentMethods,
                'paymentWindows'    => $paymentWindows,
                'adPaymentMethodId' => $adPaymentMethodId,
            ];
        }

        $data['ad'] = $ad;

        $notify[] = 'Create Advertisement';
        return responseSuccess('create_advertisement', $notify, $data);
    }
    public function save(Request $request, $id = 0)
    {
        $request->validate([
            'step' => 'required|in:1,2,3',
        ]);
        $step       = $request->step;
        $steps      = [1 => 'stepOne', 2 => 'stepTwo', 3 => 'stepThree'];
        $methodName = $steps[$step];
        return $this->$methodName($request, $id);
    }

    public function stepOne($request, $id)
    {
        $validator = Validator::make($request->all(), [
            'type'  => 'required|in:' . Status::P2P_AD_TYPE_SELL . ',' . Status::P2P_AD_TYPE_BUY . '',
            'asset' => 'required|integer',
            'fiat'  => 'required|integer',
        ]);

        if ($validator->fails()) {
            return responseError('validation_error', $validator->errors()->all());
        }

        $asset = Currency::crypto()->active()->where('id', $request->asset)->first();
        if (!$asset) {
            $notify[] = 'Crypto currency not found';
            return responseError('not_found', $notify);
        }
        $fiat = Currency::fiat()->active()->where('rate', '>', 0)->where('id', $request->fiat)->first();
        if (!$fiat) {
            $notify[] = 'Fiat currency not found';
            return responseError('not_found', $notify);
        }
        $user = auth()->user();

        if ($id) {
            $ad = Ad::where('id', $id)->where('user_id', $user->id)->first();
            if(!$ad) {
                $notify[] = 'Ad not found';
                return responseError('not_found', $notify);
            }

            if ($ad->complete_step == 3) {
                $notify[] = 'Ad already completed';
                return responseError('ad_already_completed', $notify);
            }
        } else {
            $ad                = new Ad();
            $ad->complete_step = 1;
            $ad->user_id       = $user->id;
        }
        $ad->type     = $request->type;
        $ad->asset_id = $asset->id;
        $ad->fiat_id  = $fiat->id;
        $ad->save();

        $notify[] = 'Ad step one completed successfully';
        return responseSuccess('step_one_completed', $notify, ['ad' => $ad]);
    }

    public function stepTwo($request, $id)
    {
        $validator = Validator::make($request->all(), [
            'price_type'       => 'required|in:' . Status::P2P_AD_PRICE_TYPE_FIXED . ',' . Status::P2P_AD_PRICE_TYPE_MARGIN . '',
            'price'            => 'required|numeric|gte:0',
            'payment_window'   => 'required|integer',
            'minimum_amount'   => 'required|numeric|gt:0',
            'maximum_amount'   => 'required|numeric|gt:minimum_amount',
            'margin'           => 'required_if:price_type,' . Status::P2P_AD_PRICE_TYPE_MARGIN . '|gt:0',
            'payment_method'   => 'required|array|min:1',
            'payment_method.*' => 'required',
        ]);

        if ($validator->fails()) {
            return responseError('validation_error', $validator->errors()->all());
        }

        $user = auth()->user();
        $ad   = Ad::where('id', $id)->where('user_id', $user->id)->first();
        if (!$ad) {
            $notify[] = 'Ad not found';
            return responseError('not_found', $notify);
        }
        $paymentWindow = PaymentWindow::active()->where('id', $request->payment_window)->first();
        if (!$paymentWindow) {
            $notify[] = 'Payment window not found';
            return responseError('not_found', $notify);
        }
        $paymentMethod       = PaymentMethod::active()->whereIn('id', $request->payment_method)->count();
        $paymentMethodExists = UserPaymentMethod::whereIn('payment_method_id', $request->payment_method)->where('user_id', $user->id)->count();

        if (count($request->payment_method) != $paymentMethod) {
            $notify[] = 'Payment window not found';
            return responseError('not_found', $notify);
        }

        if ($ad->type == Status::P2P_AD_TYPE_SELL && count($request->payment_method) != $paymentMethodExists) {
            $notify[] = 'Please add all payment method information first';
            return responseError('add_payment_method_first', $notify);
        }

        if ($request->price_type == Status::P2P_AD_PRICE_TYPE_FIXED) {
            $price = $request->price;
        } else {
            $coinPrice     = @$ad->asset->marketData->price;
            $currencyPrice = @$ad->fiat->rate ?? 0;
            $price         = (($coinPrice / $currencyPrice) / 100) * $request->margin;
        }

        $ad->payment_window_id = $paymentWindow->id;
        $ad->price_type        = $request->price_type;
        $ad->price             = $price;
        $ad->minimum_amount    = $request->minimum_amount;
        $ad->maximum_amount    = $request->maximum_amount;
        $ad->price_margin      = $request->margin ?? 0;

        if (1 == $ad->complete_step) {
            $ad->complete_step = 2;
        }
        $ad->save();

        $this->paymentMethods($request, $ad->id);

        $notify[] = 'Ad step two completed successfully';
        return responseSuccess('step_two_completed', $notify, ['ad' => $ad]);
    }

    public function stepThree($request, $id = 0)
    {
        $validator = Validator::make($request->all(), [
            'payment_details'  => 'required|string',
            'terms_of_trade'   => 'required|string',
            'auto_replay_text' => 'nullable|string',
        ]);

        if ($validator->fails()) {
            return responseError('validation_error', $validator->errors()->all());
        }

        $purifier = new \HTMLPurifier();
        $user     = auth()->user();
        $ad       = Ad::where('id', $id)->where('user_id', $user->id)->first();

        if (!$ad) {
            $notify[] = 'Ad not found';
            return responseError('not_found', $notify);
        }

        $ad->payment_details  = $purifier->purify($request->payment_details);
        $ad->terms_of_trade   = $purifier->purify($request->terms_of_trade);
        $ad->auto_replay_text = $purifier->purify($request->auto_replay_text);

        if ($ad->complete_step = 2) {
            $ad->complete_step = 3;
            $notify[]          = 'Ad created successfully. By clicking the below button create more ads.';
        } else {
            $notify[] = 'Ad updated successfully';
        }

        $ad->save();

        return responseSuccess('step_three_completed', $notify);
    }

    private function paymentMethods($request, $id)
    {
        $oldPaymentMethod     = AdPaymentMethod::where('ad_id', $id)->pluck('payment_method_id')->toArray() ?? [];
        $requestPaymentMethod = $request->payment_method;
        $now                  = now();

        $removePaymentMethods = array_diff($oldPaymentMethod, $requestPaymentMethod);
        $addPaymentMethods    = array_diff($requestPaymentMethod, $oldPaymentMethod);

        if ($removePaymentMethods) {
            AdPaymentMethod::where('ad_id', $id)->whereIn('payment_method_id', $removePaymentMethods)->delete();
        }

        if ($addPaymentMethods) {
            foreach ($addPaymentMethods as $paymentMethod) {
                $paymentMethods[] = [
                    'payment_method_id' => $paymentMethod,
                    'ad_id'             => $id,
                    'created_at'        => $now,
                    'updated_at'        => $now,
                ];
            }
            AdPaymentMethod::insert($paymentMethods);
        }
    }

    public function changeStatus($id)
    {
        $user = auth()->user();
        $ad   = Ad::where('id', $id)->where('user_id', $user->id)->first();

        if (!$ad) {
            $notify[] = 'Ad not found';
            return responseError('not_found', $notify);
        }

        if ($ad->status == Status::ENABLE) {
            $ad->status = Status::DISABLE;
            $message    = "Ad disabled successfully";
        } else {
            $ad->status = Status::ENABLE;
            $message    = "Ad enabled successfully";
        }
        $ad->save();

        $notify[] = $message;
        return responseSuccess('status_changed', $notify);
    }
}
